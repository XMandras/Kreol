...
st.image("https://cdn-icons-png.flaticon.com/512/2830/2830284.png", width=80) st.title("🇲🇺 Deutsch ➔ Kreol")
...
if source_text and source_text != st.session_state.last_text: with st.spinner('Der Lehrer analysiert die Nuancen...'): system_msg = "Du bist ein mauritischer Sprachexperte. Übersetze den deutschen Text in Mauritisches Kreol in 5 EXTREM unterschiedlichen Ebenen: 1. GEHOBEN: Maximal förmlich. 2. NEUTRAL: Höfliches Standard-Kreol. 3. POPULÄR: Jung, modern. 4. UMGANG: Sehr locker, viel Slang. 5. VULGÄR: Extrem derb, aggressiv und beleidigend. Format: LABEL: [Kreolisch] | [Deutsche Rückübersetzung]" response = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": system_msg}, {"role": "user", "content": source_text}]) raw_output = response.choices[0].message.content new_data = {} for line in raw_output.strip().split('\n'): if ":" in line and "|" in line: label_part, content = line.split(":", 1) t, b = content.split("|", 1) new_data[label_part.strip().upper()] = {"t": t.strip(), "b": b.strip()} st.session_state.data = new_data st.session_state.last_text = source_text
...
st.markdown("---") query_text = st.text_input("💬 Rückfrage an den Lehrer:", key=f"query_{st.session_state['text_input_key']}") if query_text and source_text: res = client.chat.completions.create(model="gpt-4o", messages=[{"role": "system", "content": "Du bist Lehrer für Mauritisches Kreol."}, {"role": "user", "content": f"Frage zu '{source_text}': {query_text}"}]) st.success(res.choices[0].message.content)
